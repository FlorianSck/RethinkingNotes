...
while(found == FALSE) {
  ...
  if(value <= 0.05) {
    value <- round(value, 4)
    print(paste0("PI with width = ", value," found for N = ", N))
    found <- TRUE
  } else {
    N <- N + 100
  }
...
is prbably the easiest way in this case. Does it look like the model fits the 
...
#### 3H5
The model assumes that sex of first and second births are independent. To check
this assumption, focus now on second births that followed female first borns.
Compare 10,000 simulated conts of boys to only those second births that followed 
girls. To do this correctly, you need to count the number of first borns who were girls 
and simulate that many births, 10,000 times. Compare the counts of boys in your 
simulations to the actual observed count of boys following girls. 
